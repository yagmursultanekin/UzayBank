...
*Clean Architecture · JWT Authentication · Rol Tabanlı Yetkilendirme · Çok Dilli · VakıfBank Open Banking · Blockchain-Ready*
...
UzayBank; kullanıcıların banka hesaplarını görüntüleyebildiği, hesap hareketlerini takip edebildiği, finansal analizlerini inceleyebildiği, güncel piyasa verilerine ve en yakın ATM/şube bilgilerine erişebildiği tam kapsamlı (full-stack) bir fintech web uygulamasıdır. Proje, **VakıfBank Open Banking API'leri** ile canlı entegrasyona sahiptir.

Uygulama, **rol tabanlı yetkilendirme** ile yönetici ve kullanıcı ayrımı yapar; yöneticiler hesapları kullanıcılara atayabilir, kullanıcılar ise yalnızca kendilerine atanmış hesapları görebilir. Arayüz **Türkçe ve İngilizce** olmak üzere iki dili destekler.

Ayrıca proje, lisans bitirme tezi kapsamında **blockchain tabanlı veri bütünlüğü** (SHA-256 hash + Ethereum) ve **Zero Trust erişim kontrolü** (akıllı kontratlar) mimarisiyle genişletilmek üzere tasarlanmıştır.
...
- 🗂 **Yönetici paneli** — yöneticiler hesapları kullanıcılara atar/kaldırır; her hesap yalnızca tek bir kullanıcıya atanabilir
...
- 📊 **Dashboard** — hesap kartları, toplam bakiye özeti
...
```bash
# Repo'yu klonla
git clone https://github.com/KULLANICI/UzayBank.git
cd UzayBank
...
| `GET` | `/api/account/my-accounts` | Giriş yapan kullanıcının hesapları | ✅ |
